...
export function Dashboard() {
    ...
    // Agrupar credenciales por cliente para la nueva vista organizada
    const groupedCredentials = credentials.reduce((acc, cred) => {
        const clientName = cred.client || 'General / Otros';
        if (!acc[clientName]) acc[clientName] = [];
        acc[clientName].push(cred);
        return acc;
    }, {} as Record<string, Credential[]>);

    const sortedClients = Object.keys(groupedCredentials).sort((a, b) => {
        if (a === 'General / Otros') return 1; // Mandar 'General' siempre al final
        if (b === 'General / Otros') return -1;
            ...
            <div className="md:hidden space-y-4">
                {loading ? (
                    ...
                    sortedClients.map(clientName => (
                        <div key={`mobile-group-${clientName}`} className="space-y-4">
                            <div className="flex items-center gap-2 mt-6 mb-2 px-1">
                                <Users className="w-4 h-4 text-[var(--text-primary)]" />
                                <h3 className="font-semibold text-[var(--text-primary)] opacity-90">{clientName}</h3>
                                <span className="bg-[var(--bg-dark)] text-[var(--text-secondary)] text-xs px-2 py-0.5 rounded-full border border-[var(--border-color)]">
                                    {groupedCredentials[clientName].length}
                                </span>
                            </div>

                            {groupedCredentials[clientName].map(cred => {
                ...
                <table className="w-full text-left border-collapse min-w-[600px]">
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            sortedClients.map(clientName => (
                                <React.Fragment key={`desktop-group-${clientName}`}>
                                    {/* Client Header Row */}
                                    <tr className="bg-[var(--bg-dark)]/50 border-b border-t border-[var(--border-color)]">
                                        <td colSpan={4} className="py-3 px-6">
                                            <div className="flex items-center gap-2">
                                                <Users className="w-4 h-4 text-[var(--accent-green)]" />
                                                <span className="font-semibold text-[var(--text-primary)] tracking-wide">{clientName}</span>
                                                <span className="bg-[var(--bg-surface)] text-[var(--text-secondary)] text-[10px] px-2 py-0.5 rounded-full border border-[var(--border-color)] ml-1">
                                                    {groupedCredentials[clientName].length}
                                                </span>
                                            </div>
                                        </td>
